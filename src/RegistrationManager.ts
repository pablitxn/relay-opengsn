...
export class RegistrationManager {
  ...
  contractInteractor: any // TODO fixeado a mano
  ...
  set isHubAuthorized(newValue: boolean) {
    ...
    if (newValue !== oldValue) {
      this.logger.info(
        `Current RelayHub is ${newValue ? "now" : "no longer"} authorized`
      )
  ...
  async init(): Promise<void> {
    ...
    this.balanceRequired = new AmountRequired(
      "Balance",
      toBN(this.config.managerMinBalance),
      this.logger,
      listener
    )
    this.stakeRequired = new AmountRequired(
      "Stake",
      toBN(this.config.managerMinStake),
      this.logger,
      listener,
      tokenMetadata
    )
  ...
  async updateLatestRegistrationTxs(
      ...
      switch (eventData.event) {
        case RelayServerRegistered:
          if (
            ...
            this.logger.debug(
              "New lastMinedRegisterTransaction: " + JSON.stringify(eventData)
            )
            const block = await this.contractInteractor.getBlock(
        ...
        case RelayWorkersAdded:
          if (
            ...
            this.logger.debug(
              "New lastWorkerAddedTransaction: " + JSON.stringify(eventData)
            )
            const block = await this.contractInteractor.getBlock(
              eventData.blockNumber
            )
  ...
  async handlePastEvents(
    ...
    const decodedEvents =
      await this.contractInteractor.getPastEventsForStakeManager(
        eventNames,
        topics,
        options
      )
      ...
      if (this.balanceRequired.isSatisfied) {
        ...
        if (!this._isSetOwnerCalled) {
          ...
          transactionHashes = transactionHashes.concat(
            await this.setOwnerInStakeManager(
              currentBlockNumber,
        ...
        this.logger.debug(
          "owner is not set and balance requirement is not satisfied"
        )
    ...
    for (const eventData of decodedEvents) {
      switch (eventData.event) {
        case HubAuthorized:
          this.logger.warn(
            `Handling HubAuthorized event: ${JSON.stringify(
              eventData
            )} in block ${currentBlockNumber}`
          )
        ...
        case OwnerSet:
          await this.refreshStake()
          this.logger.warn(
            `Handling OwnerSet event: ${JSON.stringify(
              eventData
            )} in block ${currentBlockNumber}`
          )
          break
        case StakeAdded:
          await this.refreshStake()
          this.logger.warn(
            `Handling StakeAdded event: ${JSON.stringify(
              eventData
            )} in block ${currentBlockNumber}`
          )
          break
        case HubUnauthorized:
          this.logger.warn(
            `Handling HubUnauthorized event: ${JSON.stringify(
              eventData
            )} in block ${currentBlockNumber}`
          )
          if (isSameAddress(eventData.returnValues.relayHub, this.hubAddress)) {
            ...
            this.delayedEvents.push({
              time: eventData.returnValues.removalTime.toString(),
              eventData,
            })
          }
          break
        case StakeUnlocked:
          this.logger.warn(
            `Handling StakeUnlocked event: ${JSON.stringify(
              eventData
            )} in block ${currentBlockNumber}`
          )
          await this.refreshStake()
        ...
        case StakeWithdrawn:
          this.logger.warn(
            `Handling StakeWithdrawn event: ${JSON.stringify(
              eventData
            )} in block ${currentBlockNumber}`
          )
          await this.refreshStake()
    ...
    const currentBlockObject = await this.contractInteractor.getBlock(
      currentBlockNumber
    )
    ...
    for (const eventData of this._extractDuePendingEvents(currentBlockTime)) {
      switch (eventData.event) {
        case HubUnauthorized:
          transactionHashes = transactionHashes.concat(
            await this._handleHubUnauthorizedEvent(
              eventData,
              currentBlockNumber,
              currentBlockTimestamp
            )
    ...
    if (
      ...
      this.logger.debug(
        `will attempt registration: isRegistrationPending=${isRegistrationPending} isRegistrationCorrect=${isRegistrationCorrect} forceRegistration=${forceRegistration}`
      )
      transactionHashes = transactionHashes.concat(
  ...
  _extractDuePendingEvents(currentBlockTime: number | string): EventData[] {
    ...
    const ret = this.delayedEvents
      .filter((event) => event.time <= currentBlockTimeNumber)
      .map((e) => e.eventData)
    this.delayedEvents = [
      ...this.delayedEvents.filter(
        (event) => event.time > currentBlockTimeNumber
      ),
    ]
    return ret
  }
  ...
  async _queryLatestRegistrationEvent(): Promise<CachedEventData | undefined> {
    ...
    const registerEvents =
      await this.contractInteractor.getPastEventsForRegistrar(
        [topics],
        {
  ...
  async _handleHubAuthorizedEvent(dlog: EventData): Promise<void> {
    if (
      dlog.returnValues.relayHub.toLowerCase() === this.hubAddress.toLowerCase()
    ) {
      this.isHubAuthorized = true
    }
  }

  async _handleHubUnauthorizedEvent(
    dlog: EventData,
    currentBlockNumber: number,
    currentBlockTimestamp: number
  ): Promise<PrefixedHexString[]> {
    return await this.withdrawAllFunds(
      false,
      currentBlockNumber,
      currentBlockTimestamp
    )
  ...
  async withdrawAllFunds(
    ...
    let transactionHashes: PrefixedHexString[] = []
    transactionHashes = transactionHashes.concat(
      await this._sendManagerHubBalanceToOwner(
        currentBlockNumber,
        currentBlockTimestamp
      )
    )
    transactionHashes = transactionHashes.concat(
      await this._sendWorkersEthBalancesToOwner(
        currentBlockNumber,
        currentBlockTimestamp
      )
    )
    if (withdrawManager) {
      transactionHashes = transactionHashes.concat(
        await this._sendManagerEthBalanceToOwner(
          currentBlockNumber,
          currentBlockTimestamp
        )
      )
  ...
  async refreshStake(): Promise<void> {
    ...
    const stakedOnHubStatus =
      await this.contractInteractor.isRelayManagerStakedOnHub(
        this.managerAddress
      )
    ...
    if (
      this._isOwnerSetOnStakeManager &&
      !isSameAddress(stakeInfo.owner, this.config.ownerAddress)
    ) {
      throw new Error(
        `This Relay Manager has set owner to already! On-chain: ${stakeInfo.owner}, in config: ${this.config.ownerAddress}`
      )
    }
    if (stake.eq(toBN(0))) {
      return
  ...
  async attemptRegistration(
    ...
    const stakeOnHubStatus =
      await this.contractInteractor.isRelayManagerStakedOnHub(
        this.managerAddress
      )
    if (!stakeOnHubStatus.isStaked && this.ownerAddress != null) {
      this.logger.error(
        "Relay manager is staked on StakeManager but not on RelayHub."
      )
      this.logger.error(
        "Minimum stake/minimum unstake delay/stake token misconfigured?"
      )
    }
    ...
    if (!allPrerequisitesOk) {
      this.logger.debug(
        "will not actually attempt registration - prerequisites not satisfied"
      )
      return []
    }
    ...
    if (!(workersAdded || addWorkersPending)) {
      const txHash = await this.addRelayWorker(
        currentBlockNumber,
        currentBlockTimestamp
      )
    ...
    const registerMethod = await this.contractInteractor.getRegisterRelayMethod(
      this.hubAddress,
      this.config.baseRelayFee,
      this.config.pctRelayFee,
      this.config.url
    )
  ...
  async _sendManagerEthBalanceToOwner(
    ...
    if (managerBalance.gte(txCost)) {
      this.logger.info(
        `Sending manager eth balance ${managerBalance.toString()} to owner`
      )
      const details: SendTransactionDetails = {
        signer: this.managerAddress,
  ...
  async _sendWorkersEthBalancesToOwner(
    ...
    if (workerBalance.gte(txCost)) {
      this.logger.info(
        `Sending workers' eth balance ${workerBalance.toString()} to owner`
      )
      const details: SendTransactionDetails = {
  ...
  async _sendManagerHubBalanceToOwner(
    ...
    const transactionHashes: PrefixedHexString[] = []
    const gasPrice = await this.contractInteractor.getGasPrice()
    const managerHubBalance = await this.contractInteractor.hubBalanceOf(
      this.managerAddress
    )
    if (amount == null) {
      ...
      throw new Error(
        `Withdrawal amount ${amount.toString()} larger than manager hub balance ${managerHubBalance.toString()}`
      )
    }
    const { gasLimit, gasCost, method } =
      await this.contractInteractor.withdrawHubBalanceEstimateGas(
        amount,
        this.ownerAddress,
        this.managerAddress,
        gasPrice
    ...
    if (amount.gte(gasCost)) {
      this.logger.info(
        `Sending manager hub balance ${amount.toString()} to owner`
      )
      const details: SendTransactionDetails = {
        gasLimit,
        signer: this.managerAddress,
      ...
      this.logger.error(
        `manager hub balance too low: ${managerHubBalance.toString()}, tx cost: ${gasCost.toString()}`
      )
    }
    return transactionHashes
  }
  ...
  async _isWorkerValid(): Promise<boolean> {
    const managerFromHub = await this.contractInteractor.workerToManager(
      this.workerAddress
    )
    ...
    return (
      this.lastWorkerAddedTransaction != null &&
      this.lastWorkerAddedTransaction.eventData.returnValues.newRelayWorkers
...
`)
  ...
  async setOwnerInStakeManager(
    currentBlockNumber: number,
    currentBlockTimestamp: number
  ): Promise<PrefixedHexString> {
    const setRelayManagerMethod =
      await this.contractInteractor.getSetRelayManagerMethod(
    ...
    this.logger.info(
      `setting relay owner ${this.config.ownerAddress} at StakeManager ${stakeManagerAddress}`
    )
    const { transactionHash } = await this.transactionManager.sendTransaction(
      details
    )
