...
export async function resolveServerConfig(
  ...
  if (config.environmentName != null) {
    ...
    console.error(
      `Must provide one of the supported values for environmentName: ${Object.keys(
        EnvironmentsKeys
      ).toString()}`
    )
  }
  ...
  if (
    ...
    config.ownerAddress === constants.ZERO_ADDRESS
  )
    error("missing param: ownerAddress")
  if (
    config.managerStakeTokenAddress == null ||
    config.managerStakeTokenAddress === constants.ZERO_ADDRESS
  )
    error("missing param: managerStakeTokenAddress")
...
export function validateBalanceParams(config: ServerConfigParams): void {
  ...
  if (minHubWithdrawalBalance.gt(withdrawToOwnerOnBalance)) {
    throw new Error(
      "withdrawToOwnerOnBalance must be at least minHubWithdrawalBalance"
    )
  }
  if (
    managerTargetBalance.add(workerTargetBalance).gte(withdrawToOwnerOnBalance)
  ) {
    throw new Error(
      "withdrawToOwnerOnBalance must be larger than managerTargetBalance + workerTargetBalance"
    )
  }
}

export function resolveReputationManagerConfig(
  config: any
): Partial<ReputationManagerConfiguration> {
