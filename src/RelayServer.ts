...
import {
  ReadinessInfo,
  StatsResponse,
} from "@opengsn/common/dist/StatsResponse"
...
import {
  gsnRequiredVersion,
  gsnRuntimeVersion,
} from "@opengsn/common/dist/Version"
import {
  address2topic,
  decodeRevertReason,
  getLatestEventData,
...
export class RelayServer extends EventEmitter {
  ...
  relayHubContract!: any // TODO fixeado a mano

  trustedPaymastersGasAndDataLimits: Map<
    String | undefined,
    PaymasterGasAndDataLimits
  > = new Map<String | undefined, PaymasterGasAndDataLimits>()
  ...
  statsHandler(): StatsResponse {
    ...
    const statsResponse: StatsResponse = {
      ...this.readinessInfo,
      totalUptime: now - this.readinessInfo.runningSince,
    }
    if (this.isReady()) {
      statsResponse.totalReadyTime =
        this.readinessInfo.totalReadyTime +
        now -
        this.readinessInfo.currentStateTimestamp
    } else {
      statsResponse.totalNotReadyTime =
        this.readinessInfo.totalNotReadyTime +
        now -
        this.readinessInfo.currentStateTimestamp
    }
  ...
  validateRequestTxType(req: RelayTransactionRequest): void {
    if (
      ...
      req.relayRequest.relayData.maxFeePerGas !==
        req.relayRequest.relayData.maxPriorityFeePerGas
    ) {
      throw new Error(
        `Network ${this.contractInteractor.getNetworkType()} doesn't support eip1559`
      )
    }
  }
  ...
  validateInput(req: RelayTransactionRequest): void {
    ...
    if (this._isBlacklistedPaymaster(req.relayRequest.relayData.paymaster)) {
      throw new Error(
        `Paymaster ${req.relayRequest.relayData.paymaster} is blacklisted!`
      )
    }
  ...
  validateGasFees(req: RelayTransactionRequest): void {
    const requestPriorityFee = parseInt(
      req.relayRequest.relayData.maxPriorityFeePerGas
    )
  ...
  validateRelayFees(req: RelayTransactionRequest): void {
    ...
    if (
      parseInt(req.relayRequest.relayData.pctRelayFee) < this.config.pctRelayFee
    ) {
      throw new Error(
    ...
    if (
      toBN(req.relayRequest.relayData.baseRelayFee).lt(
        toBN(this.config.baseRelayFee)
      )
    ) {
      throw new Error(
  ...
  async validatePaymasterReputation(
    paymaster: Address,
    ...
    const status = await this.reputationManager.getPaymasterStatus(
      paymaster,
      currentBlockNumber
    )
    ...
    switch (status) {
      case PaymasterStatus.ABUSED:
        message =
          "This paymaster has failed a lot of transactions recently is temporarily blocked by this relay"
        break
      ...
      case PaymasterStatus.BLOCKED:
        message =
          "This paymaster had too many unsuccessful transactions and is now permanently blocked by this relay"
        break
    }
    throw new Error(
      `Refusing to serve transactions for paymaster at ${paymaster}: ${message}`
    )
  }
  ...
  async validatePaymasterGasAndDataLimits(
    ...
    const relayTransactionCalldataGasUsedCalculation =
      this.contractInteractor.calculateCalldataCost(msgData)
    const message =
    ...
    if (
      toBN(relayTransactionCalldataGasUsedCalculation).gt(
        toBN(req.relayRequest.relayData.transactionCalldataGasUsed)
      )
    ) {
      throw new Error(
        `Refusing to relay a transaction due to calldata cost. ${message}`
      )
    }
    ...
    if (gasAndDataLimits == null) {
      try {
        const paymasterContract =
          await this.contractInteractor._createPaymaster(paymaster)
      ...
      const msgDataGasCostInsideTransaction =
        msgDataLength * this.environment.dataOnChainHandlingGasCostPerByte
      const paymasterAcceptanceBudget = toNumber(
        gasAndDataLimits.acceptanceBudget
      )
      const effectiveAcceptanceBudget =
        paymasterAcceptanceBudget +
        msgDataGasCostInsideTransaction +
        relayTransactionCalldataGasUsedCalculation
      if (effectiveAcceptanceBudget > acceptanceBudget) {
        ...
        this.logger.debug(
          `Using trusted paymaster's higher than max acceptance budget: ${paymasterAcceptanceBudget}`
        )
    ...
    const tmpMaxPossibleGas =
      this.contractInteractor.calculateTransactionMaxPossibleGas({
        msgData,
        gasAndDataLimits,
    ...
    const maxPossibleGas =
      GAS_RESERVE + Math.floor(tmpMaxPossibleGas * GAS_FACTOR)
    if (maxPossibleGas > this.maxGasLimit) {
      throw new Error(
        `maxPossibleGas (${maxPossibleGas}) exceeds maxGasLimit (${this.maxGasLimit})`
      )
    }
    const maxCharge = await this.relayHubContract.calculateCharge(
      maxPossibleGas,
      req.relayRequest.relayData,
    ...
    if (paymasterBalance.lt(maxCharge)) {
      throw new Error(
        `paymaster balance too low: ${paymasterBalance.toString()}, maxCharge: ${maxCharge.toString()}`
      )
    }
    this.logger.debug(
      `paymaster balance: ${paymasterBalance.toString()}, maxCharge: ${maxCharge.toString()}`
    )
    this.logger.debug(
      `Estimated max charge of relayed tx: ${maxCharge.toString()}, GasLimit of relayed tx: ${maxPossibleGas}`
    )
...
paymasterAccepted  | ${
      viewRelayCallRet.paymasterAccepted
        ? chalk.green("true")
        : chalk.red("false")
    }
...
`)
    if (!viewRelayCallRet.paymasterAccepted) {
      throw new Error(
        `Paymaster rejected in server: ${decodeRevertReason(
          viewRelayCallRet.returnValue
  ...
  async createRelayTransaction(
    ...
    if (this.alerted) {
      ...
      await sleep(
        randomInRange(
          this.config.minAlertedDelayMS,
          this.config.maxAlertedDelayMS
        )
      )
    }
  ...
  async _initTrustedPaymasters(paymasters: string[] = []): Promise<void> {
    ...
    for (const paymasterAddress of paymasters) {
      const paymaster = await this.contractInteractor._createPaymaster(
        paymasterAddress
      )
      const gasAndDataLimits = await paymaster
        .getGasAndDataLimits()
        .catch((e: Error) => {
          throw new Error(
            `not a valid paymaster address in trustedPaymasters list: ${paymasterAddress}: ${e.message}`
          )
        })
      this.trustedPaymastersGasAndDataLimits.set(
  ...
  _getPaymasterMaxAcceptanceBudget(paymaster?: string): IntString {
    const limits = this.trustedPaymastersGasAndDataLimits.get(
  ...
  async init(): Promise<void> {
    ...
    if (
      this.config.coldRestartLogsFromBlock == null ||
      latestBlock.number < this.config.coldRestartLogsFromBlock
    ) {
      throw new Error(
    ...
    if (this.config.devMode && (this.chainId < 1000 || this.networkId < 1000)) {
      this.logger.error(
        "Don't use real network's chainId & networkId while in devMode."
      )
      process.exit(-1)
    }
...
`)
    ...
    this.logger.debug(
      `server init finished in ${Date.now() - initStartTimestamp} ms`
    )
  }
  ...
  async replenishServer(
    ...
    const managerHubBalance = await this.relayHubContract.balanceOf(
      this.managerAddress
    )
    this.workerBalanceRequired.currentValue = await this.getWorkerBalance(
      workerIndex
    )
    if (
      managerEthBalance.gte(
        toBN(this.config.managerTargetBalance.toString())
      ) &&
      this.workerBalanceRequired.isSatisfied
    ) {
      // all filled, nothing to do
      return transactionHashes
    }
    const mustWithdrawHubDeposit =
      managerEthBalance.lt(toBN(this.config.managerTargetBalance.toString())) &&
      managerHubBalance.gte(toBN(this.config.minHubWithdrawalBalance))
    const isWithdrawalPending =
      await this.txStoreManager.isActionPendingOrRecentlyMined(
        ServerAction.DEPOSIT_WITHDRAWAL,
        currentBlockNumber,
        this.config.recentActionAvoidRepeatDistanceBlocks
      )
    if (mustWithdrawHubDeposit && !isWithdrawalPending) {
      this.logger.info(
        `withdrawing manager hub balance (${managerHubBalance.toString()}) to manager`
      )
      // Refill manager eth balance from hub balance
      const method = this.relayHubContract?.contract.methods.withdraw(
        this.managerAddress,
        toHex(managerHubBalance)
      )
      const details: SendTransactionDetails = {
      ...
      const { transactionHash } = await this.transactionManager.sendTransaction(
        details
      )
    ...
    if (mustReplenishWorker && !isReplenishPendingForWorker) {
      const refill = toBN(this.config.workerTargetBalance.toString()).sub(
        this.workerBalanceRequired.currentValue
      )
      this.logger.debug(
      ...
      if (
        refill.lt(managerEthBalance.sub(toBN(this.config.managerMinBalance)))
      ) {
  ...
  async intervalHandler(): Promise<void> {
    try {
      ...
      if (blockNumber > this.lastScannedBlock) {
        await this._worker(blockNumber).then((transactions) => {
  ...
  async _refreshPriorityFee(): Promise<void> {
    const minMaxPriorityFeePerGas = parseInt(
      await this.contractInteractor.getMaxPriorityFee()
    )
    this.minMaxPriorityFeePerGas = Math.floor(
      minMaxPriorityFeePerGas * this.config.gasPriceFactor
    )
    if (this.minMaxPriorityFeePerGas === 0) {
      this.logger.debug(
        `Priority fee received from node is 0. Setting priority fee to ${this.config.defaultPriorityFee}`
      )
    ...
    if (this.minMaxPriorityFeePerGas > parseInt(this.config.maxGasPrice)) {
      throw new Error(
        `network maxPriorityFeePerGas ${this.minMaxPriorityFeePerGas} is higher than config.maxGasPrice ${this.config.maxGasPrice}`
      )
    }
  }

  async _handleChanges(
    currentBlockNumber: number,
    currentBlockTimestamp: number
  ): Promise<PrefixedHexString[]> {
    ...
    await this.registrationManager.updateLatestRegistrationTxs(
    ...
    await this._boostStuckPendingTransactions(
      currentBlockNumber,
      currentBlockTimestamp
    )
    ...
    if (
      this.alerted &&
      this.alertedByTransactionBlockTimestamp +
        this.config.alertedDelaySeconds <
        currentBlockTimestamp
    ) {
      this.logger.warn(
        `Relay exited alerted state. Alerted transaction timestamp: ${this.alertedByTransactionBlockTimestamp}. Current block timestamp: ${currentBlockTimestamp}`
      )
      this.alerted = false
    }
    return transactionHashes
  ...
  async getManagerBalance(): Promise<BN> {
    return toBN(
      await this.contractInteractor.getBalance(this.managerAddress, "pending")
    )
  }

  async getWorkerBalance(workerIndex: number): Promise<BN> {
    return toBN(
      await this.contractInteractor.getBalance(this.workerAddress, "pending")
    )
  }
  ...
  async _shouldRegisterAgain(
    ...
    const relayRegistrationMaxAge =
      await this.contractInteractor.getRelayRegistrationMaxAge()
    const latestRegisterTxBlockTimestamp =
      this._getLatestRegisterTxBlockTimestamp()
    ...
    const registrationExpired =
      currentBlockTimestamp - latestRegisterTxBlockTimestamp >=
        relayRegistrationMaxAge.toNumber() && !isPendingRegistration
    ...
    for (const event of hubEventsSinceLastScan) {
      switch (event.event) {
        case TransactionRejectedByPaymaster:
          this.logger.debug(
            `handle TransactionRejectedByPaymaster event: ${JSON.stringify(
              event
            )}`
          )
          await this._handleTransactionRejectedByPaymasterEvent(
            event.returnValues.paymaster,
            currentBlockNumber,
            event.blockNumber
          )
          break
        case TransactionRelayed:
          ...
          await this._handleTransactionRelayedEvent(
            event.returnValues.paymaster,
            event.blockNumber
          )
          break
  ...
  async getAllHubEventsSinceLastScan(): Promise<EventData[]> {
    ...
    const hubEvents = await this.contractInteractor.getPastEventsForHub(
      topics,
      options
    )
    const regEvents = await this.contractInteractor.getPastEventsForRegistrar(
      topics,
      options
    )
  ...
  async _handleTransactionRelayedEvent(
    paymaster: Address,
    eventBlockNumber: number
  ): Promise<void> {
    if (this.config.runPaymasterReputations) {
      await this.reputationManager.updatePaymasterStatus(
        paymaster,
        true,
        eventBlockNumber
      )
  ...
  async _handleTransactionRejectedByPaymasterEvent(
    paymaster: Address,
    currentBlockNumber: number,
    eventBlockNumber: number
  ): Promise<void> {
    ...
    const alertedUntil =
      this.alertedByTransactionBlockTimestamp + this.config.alertedDelaySeconds
    this.logger
      .error(`Relay entered alerted state. Block number: ${eventBlockNumber} Block timestamp: ${eventBlockTimestamp}.
  ...
  _getLatestRegisterTxBlockTimestamp(): number {
    return (
      this.registrationManager.lastMinedRegisterTransaction?.blockTimestamp ??
      -1
    )
  }

  async _updateLatestTxBlockNumber(
    eventsSinceLastScan: EventData[]
  ): Promise<void> {
    ...
    if (this.lastMinedActiveTransaction == null) {
      ...
      this.logger.debug(
        `queried node for last active server event, found in block ${this.lastMinedActiveTransaction?.blockNumber}`
      )
    }
  }
  ...
  async withdrawToOwnerIfNeeded(
    ...
    try {
      ...
      const reserveBalance = toBN(this.config.managerTargetBalance).add(
        toBN(this.config.workerTargetBalance)
      )
      const effectiveWithdrawOnBalance = toBN(
        this.config.withdrawToOwnerOnBalance
      ).add(reserveBalance)
      const managerHubBalance = await this.relayHubContract.balanceOf(
        this.managerAddress
      )
      ...
      txHashes = txHashes.concat(
        await this.registrationManager._sendManagerHubBalanceToOwner(
          currentBlockNumber,
          currentBlockTimestamp,
          withdrawalAmount
        )
  ...
  async _boostStuckPendingTransactions(
    ...
    const managerBoostedTransactions =
      await this._boostStuckTransactionsForManager(
        currentBlockNumber,
        currentBlockTimestamp
      )
    ...
    for (const workerIndex of [0]) {
      const workerBoostedTransactions =
        await this._boostStuckTransactionsForWorker(
          currentBlockNumber,
          currentBlockTimestamp,
          workerIndex
        )
  ...
  _isTrustedPaymaster(paymaster: string): boolean {
    return (
      this.trustedPaymastersGasAndDataLimits.get(paymaster.toLowerCase()) !=
      null
    )
  }

  _isBlacklistedPaymaster(paymaster: string): boolean {
    return this.config.blacklistedPaymasters
      .map((it) => it.toLowerCase())
      .includes(paymaster.toLowerCase())
  }
  ...
  setReadyState(isReady: boolean): void {
    if (this.isReady() !== isReady) {
      ...
      if (isReady) {
        ...
        this.readinessInfo.totalNotReadyTime +=
          now - this.readinessInfo.currentStateTimestamp
      } else {
        this.readinessInfo.totalReadyTime +=
          now - this.readinessInfo.currentStateTimestamp
